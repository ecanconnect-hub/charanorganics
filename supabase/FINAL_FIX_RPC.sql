-- ========================================================
-- ⚠️ FINAL FIX for "Function Not Unique" Error
-- ========================================================
-- Run this ENTIRE script in the Supabase SQL Editor.
-- It explicitly drops the conflicting function signatures.
-- ========================================================

-- 1. Drop the version WITHOUT user_id (Old version)
DROP FUNCTION IF EXISTS public.place_secure_order(
    text, text, text, text, text, text, public.order_item_input[], numeric, numeric, numeric
);

-- 2. Drop the version WITH user_id (Newer version)
DROP FUNCTION IF EXISTS public.place_secure_order(
    text, text, text, text, text, text, public.order_item_input[], numeric, numeric, numeric, uuid
);

-- 3. Now that conflicts are gone, re-create the correct function
...
BEGIN
    ...
    -- 🛒 NOTE: Cart clearing is removed here. Logic moved to payment confirmation.
