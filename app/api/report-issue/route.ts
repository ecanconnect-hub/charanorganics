...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        const fallbackMessageUrl = buildWhatsappMessage({
            issueType,
            orderId,
            description,
            reporterName,
        ...
        if (insertError) {
            ...
            return NextResponse.json(
                {
                    success: false,
                    error: 'We could not save your issue right now. Please contact us on WhatsApp so we can help you immediately.',
                    whatsappFallback: fallbackMessageUrl,
                },
                {
                    status: 503,
                    headers: {
        ...
        return NextResponse.json(
            {
                ...
                whatsappFallback: buildWhatsappMessage({
                    issueType,
                    orderId,
                    description,
                    reporterName,
